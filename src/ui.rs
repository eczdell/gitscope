...
fn build_status_bar_styled(app: &AppState) -> Line<'static> {
    let mode_label = match app.mode {
        ...
        crate::app::AppMode::Gists | crate::app::AppMode::GistsFilter => " gists ",
        crate::app::AppMode::GistContent => " gist content ",
        crate::app::AppMode::Repos | crate::app::AppMode::ReposAdd => " repos ",
        _ => " tree ",
    };
...
fn render_command_bar(app: &AppState) -> Paragraph<'static> {
    let text = match app.mode {
        ...
        crate::app::AppMode::Repos => format!(
...
fn render_issues_content(frame: &mut Frame, area: Rect, app: &AppState) {
    let vis = (area.height as usize).max(1);
...
fn render_repos_view(frame: &mut Frame, area: Rect, app: &AppState) {
    let vis = (area.height as usize).max(1);
...
fn render_repos_add_view(frame: &mut Frame, area: Rect, app: &AppState) {
    let mut lines: Vec<Line<'static>> = Vec::new();
...
fn render_gists_view(frame: &mut Frame, area: Rect, app: &AppState) {
    let vis = (area.height as usize).max(1);
...
fn render_gist_content_view(frame: &mut Frame, area: Rect, app: &AppState) {
    let vis = (area.height as usize).max(1);
...
pub fn ui(frame: &mut Frame, app: &AppState) {
    ...
    match app.mode {
        ...
        crate::app::AppMode::Repos => render_repos_view(frame, chunks[chunk_idx], app),
        crate::app::AppMode::ReposAdd => render_repos_add_view(frame, chunks[chunk_idx], app),
        crate::app::AppMode::Gists | crate::app::AppMode::GistsFilter => {
            render_gists_view(frame, chunks[chunk_idx], app)
        }
        crate::app::AppMode::GistContent => {
